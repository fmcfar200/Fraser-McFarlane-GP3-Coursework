...
Starfield::Starfield(GLint theTextureID, glm::vec3 theFieldVolume)
{
	srand((unsigned)time(NULL));
	setRotAngle(0.0f);
	glGetFloatv(GL_ALIASED_POINT_SIZE_RANGE, m_AliasedPointSizes);
...
void Starfield::render(float rotAngle)
{
...
void Starfield::initialise(Texture theTexture)
{
	srand((unsigned)time(NULL));
	glBindTexture(GL_TEXTURE_2D, theTexture.getTexture());
}

void Starfield::initialise(GLint theTextureID, glm::vec3 theFieldVolume)
{
	srand((unsigned)time(NULL));
	m_TextureID = theTextureID;
	m_FieldVolume = theFieldVolume;
