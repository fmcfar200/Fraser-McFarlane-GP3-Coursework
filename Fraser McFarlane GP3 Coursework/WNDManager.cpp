...
WNDManager* WNDManager::pInstance = NULL;
...
WNDManager* WNDManager::getInstance()
{
	if (pInstance == NULL)
	{
		pInstance = new WNDManager();
	}
	return WNDManager::pInstance;
}

bool WNDManager::createWND(int width, int height, int bpp)
{
	...
	m_windowClass.lpfnWndProc = (WNDPROC)WNDManager::WndProc; //We set our static method as the event handler (WNDPROC)
...
void WNDManager::destroyWND()
{
	ShowCursor(true);                       // Show Mouse Pointer
}

void WNDManager::attachOGLWnd(windowOGL* OGLWindow)
{
	m_winOGL = OGLWindow;
}

void WNDManager::attachInputMgr(InputManager* inputMgr)
{
	m_InputMgr = inputMgr;
}

bool WNDManager::isWNDRunning()
{
	return m_isRunning;
}

HWND WNDManager::getWNDHandle()               // Return window handle.
{
	return m_hwnd;
}

HDC WNDManager::getWNDDC()
{
	return m_hdc;
}

void WNDManager::processWNDEvents()
{
	MSG msg;
...
void WNDManager::setupPixelFormat(void)
{
	int pixelFormat;
...
LRESULT CALLBACK WNDManager::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	WNDManager* theWindow = NULL;
	switch (uMsg)
	{
...
windowOGL*  WNDManager::getAttachedWND()
{
	return m_winOGL;
}


float WNDManager::getElapsedSeconds()
{
