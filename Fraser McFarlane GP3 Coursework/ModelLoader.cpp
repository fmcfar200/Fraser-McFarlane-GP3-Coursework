...
void ModelLoader::loadModel(const char* mdlFilename)
{
...
void ModelLoader::loadModel(const char* mdlFilename, GLuint textureID)
{
	m_model = glmReadOBJ(mdlFilename);
	glmUnitize(m_model);
	glmFacetNormals(m_model);
...
void ModelLoader::loadModel(const char* mdlFilename, Texture mdlTexture)
{
	m_model = glmReadOBJ(mdlFilename);
	//glmUnitize(m_model);
	glmFacetNormals(m_model);
...
void ModelLoader::renderMdl(glm::vec3 mdlPosition, float mdlRotationAngle, glm::vec3 mdlScale)
{
	glPushMatrix();
...
mdlDimensions ModelLoader::getModelDimensions()
{
